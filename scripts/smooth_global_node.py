...
from path_smoothing.srv import SmoothTraj, GetSmoothPlan
...
def get_original_path(start_pose, end_pose):
    ...
    try:
        ...
        plan = get_plan(
            planner_type=GetPlan._request_class.RRT,
            starting_pose=start_pose,
            target_pose=end_pose,
            target_cost=0,
            max_time=rospy.Duration(2),
            max_iterations=0,
        )
...
def get_smooth_path(start_pose, end_pose):
    ...
    try:
        smooth_path = rospy.ServiceProxy("smooth_path", SmoothTraj)
        result = smooth_path(
            xd_mat,
            q_mat,
            r_mat,
        ...
        raise
    return (
        np.array(result.smoothed_path).reshape((ndim, nint, nstep), order="F"),
...
def _create_qrs(ndim, nint):
    ...
    s_mat = q_mat
    return (
        array_to_multi_array(q_mat),
        array_to_multi_array(r_mat),
        array_to_multi_array(s_mat),
    )
...
def _start_end(start_pt, end_pt):
    start_ps = Pose()
    start_ps.position.x, start_ps.position.y, start_ps.position.z = (
        start_pt[0],
        start_pt[1],
        0,
    )
    ...
    (
        start_ps.orientation.x,
        start_ps.orientation.y,
        start_ps.orientation.z,
        start_ps.orientation.w,
    ) = (0.0, 0.0, 0.0, 1.0)
