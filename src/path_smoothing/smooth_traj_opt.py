...
CORRIDOR_WORLD_STRAIGHT = {
    1: ((-2.0, -2.0, 14.0, 14.0), (-1.75, 1.75, 1.75, -1.75)),
    2: ((10.0, 10.0, 14.0, 14.0,), (-1.75, 10.0, 10.0, -1.75)),
    3: ((10.0, 10.0, 22.0, 22.0), (6.0, 10.0, 10.0, 6.0)),
    4: ((18.0, 18.0, 22.0, 22.0), (-8.0, 10.0, 10.0, -8)),
    5: ((0.0, 0.0, 22.0, 22.0), (-8.0, -4.0, -4.0, -8.0)),
}
...
CORRIDOR_WORLD_STRAIGHT_WITH_OBSTACLE = {
    ...
    3: (
        (10.0, 10.0, OBSTACLE_MIDDLE["tl"].x, OBSTACLE_MIDDLE["tl"].x),
        (OBSTACLE_MIDDLE["bl"].y - 1, 10, 10, OBSTACLE_MIDDLE["bl"].y - 1),
    ),
    4: (
        (10.0, 10.0, OBSTACLE_MIDDLE["tr"].x + 1, OBSTACLE_MIDDLE["tr"].x + 1),
        (OBSTACLE_MIDDLE["tl"].y, 10.0, 10.0, OBSTACLE_MIDDLE["tl"].y),
    ),
    5: (
        (OBSTACLE_MIDDLE["tr"].x, OBSTACLE_MIDDLE["tr"].x, 22.0, 22.0),
        (6.0, 10.0, 10.0, 6.0),
    ),
    6: ((18.0, 18.0, 22.0, 22.0), (-8.0, 10.0, 10.0, -8)),
    7: ((0.0, 0.0, 22.0, 22.0), (-8.0, -4.0, -4.0, -8.0)),
}
...
WORLD_SHORT = {1: CORRIDOR_WORLD_STRAIGHT[2], 2: CORRIDOR_WORLD_STRAIGHT[3]}

WORLD_SHORT_N = {
    1: {1: ((10.0, 10.0, 16.0, 22.0, 22.0), (-1.75, 3.0, 10.0, 10.0, -1.75))},
    2: {
        1: CORRIDOR_WORLD_STRAIGHT_WITH_OBSTACLE[2],
        2: CORRIDOR_WORLD_STRAIGHT_WITH_OBSTACLE[5],
    },
    3: {
        1: ((10.0, 10.0, 14.0, 14.0), (-1.75, 6.0, 6.0, -1.75)),
        2: ((13.0, 13.0, 14.0, 14.0), (6.0, 7.0, 7.0, 6.0)),
        3: ((14.0, 14.0, 22.0, 22.0,), (6.0, 10.0, 10.0, 6.0)),
    },
}

SmoothingArguments = collections.namedtuple(
    "SmoothingArguments",
    [
        "constraints",
        "cost",
...
SmoothingArgumentsObstacles = collections.namedtuple(
    "SmoothingArgumentsObstacles",
    [
        "constraints",
...
def _path_from_mat(path_matrix, time_step):
    def path(t):
        (
            ...
            path_matrix.shape
        )  # bug in pylint with nested functions pylint: disable=unused-variable
...
def smooth_unconstrained(desired_path, q_mat, r_mat, s_mat, time_step):
    ...
    smoother = smooth.SmoothTrajLinear(
        args.constraints,
        args.cost,
        args.solver,
        args.n_step,
...
def _setup_unconstrained(desired_path, q_mat, r_mat, s_mat, time_step):
    ...
    path = _path_from_mat(desired_path.reshape(-1, n_step, order="F"), time_step)
    cost = quad_cost.ContinuousQuadraticCost(q_mat, r_mat, s_mat, desired_state=path)

    term_const = lin_consts.LinearTimeInstantConstraint(
        time_step * (n_step - 1),
        (np.eye(n_dim * n_int), np.zeros((n_dim * n_int, n_dim))),
    ...
    return SmoothingArguments(
        constraints, cost, solver, n_step, initial_state, time_step, n_dim, n_int
    )


def smooth_constrained(desired_path, q_mat, r_mat, s_mat, a_mat, b_mat, time_step):
    """Smooth constrined."""
    args = _setup_constrained(
        desired_path, q_mat, r_mat, s_mat, a_mat, b_mat, time_step
    )
...
def _setup_constrained(desired_path, q_mat, r_mat, s_mat, a_mat, b_mat, time_step):
    params = _setup_unconstrained(desired_path, q_mat, r_mat, s_mat, time_step)

    a_mat = a_mat.reshape(
        -1, params.n_dim * (params.n_int + 1), params.n_step, order="F"
    )
    ...
    if USE_KEY_FRAME:
        ineq_mats = _key_frame(
            ineq_mats,
            lambda t: (
                np.empty((0, params.n_dim * params.n_int)),
                np.empty((0, params.n_dim)),
            ),
            lambda t: _indicator(t, params.time_step),
        )
        ineq_bound = _key_frame(
            ineq_bound,
            lambda t: np.empty((0, 1)),
            lambda t: _indicator(t, params.time_step),
        )
    ineq_const = lin_consts.LinearInequalityConstraint(
        ineq_mats=ineq_mats, bound=ineq_bound
    )

    constraints = lin_consts.LinearConstraints(
        ineq_constraints=[ineq_const], constraints=[params.constraints]
    )
...
def smooth_obstacles_mip(
    desired_path, q_mat, r_mat, s_mat, a_cnstr_mat, b_cnstr_mat, free_regions, time_step
):
    """Smooth the path in the presence of obstacles."""
    args = _setup_obstacles_mip(
        desired_path,
        q_mat,
        r_mat,
        s_mat,
    ...
    smoother = smooth.SmoothTrajLinearObstaclesMip(
        args.constraints,
...
def _setup_obstacles_mip(
    desired_path, q_mat, r_mat, s_mat, a_cnstr_mat, b_cnstr_mat, free_regions, time_step
):
    params = _setup_constrained(
        desired_path, q_mat, r_mat, s_mat, a_cnstr_mat, b_cnstr_mat, time_step
    )

    solver = (
        gurobi.Gurobi()
    )  # Only Gurobi can handle obstacles (Mixed Integer Programming)
    ...
    for i, region in enumerate(free_regions):
        ...
        a_rest_mat = np.zeros((a_pos_mat.shape[0], params.n_dim * (params.n_int - 1)))
        ...
        bin_const = bin_consts.ImplicationInequalityConstraint(
            i, inequality, big_m_vec, len(free_regions)
        )
        constr_list.append(bin_const)

    return SmoothingArgumentsObstacles(
        params.constraints,
        constr_list,
        params.cost,
        solver,
...
def _calc_big_m(region, limits):
    ...
    return np.max(np.abs(values), axis=1, keepdims=1) + np.abs(b_vec.reshape(-1, 1))
...
def _plot_traj_and_states(x, u, xd_mat):
    x_mat = x.reshape(xd_mat.shape[0], xd_mat.shape[1] - 1, xd_mat.shape[2], order="F")
...
def _plot_states(x, u, xd_mat, fig=None):
    ...
    for j in range(ndim):
        for i in range(nint):
            ...
            if j == 0 and i > 0:
                axes.set_ylabel(
                    [r"$\frac{d}{dt}$", r"$\frac{d^2}{dt^2}$", r"$\frac{d^3}{dt^3}$"][
                        i - 1
                    ],
                    rotation="horizontal",
                )
...
def test_constrained():
    ...
    result = smooth_constrained(
        xd_mat[:, :-1, :], q_mat, r_mat, s_mat, a_mat, b_vec, dt
    )
    _plot_traj_and_states(result[0], result[1], xd_mat)
    return result
...
def test_obstacles_mip():
    ...
    result = smooth_obstacles_mip(
        xd_mat[:, :-1, :], q_mat, r_mat, s_mat, a_mat, b_vec, free_regions, dt
    )
...
def _create_free_regions(position, dist=5.0):
    ...
    while curr_idx < position.shape[1]:
        ...
        tmp_idx, tmp_pos = _get_next_idx_pos_after(
            position, curr_pos, curr_idx, dist - dist / 20
        )
        curr_idx, curr_pos = _get_next_idx_pos_after(
            position, tmp_pos, tmp_idx, dist - dist / 20
        )
...
def _get_next_idx_pos_after(position, curr_pos, curr_idx, dist):
    ...
    try:
        ...
        out_pos = (
            curr_pos  # if the index is out of bounds, then return current position
        )
...
def _plot_free_regions(regions, fig=None, axes=None):
    ...
    poly_collection = plt.matplotlib.collections.PatchCollection(polys, alpha=0.4)
...
def test_obstacles_short():
    ...
    xd_mat, q_mat, r_mat, s_mat, a_mat, b_vec, dt = _constrained_from_mat(
        _load(), start, stop, step
    )
    if CREATE_FREE_REGIONS:
        free_regions = _create_free_regions(np.squeeze(xd_mat[:, 0, :]))
    else:
